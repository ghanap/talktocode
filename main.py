        ...
        for file in files:
            ...
            if ext in valid_extensions:
                ...
                try:
                    ...
                    docs.append(
                        Document(page_content=content, metadata={"source": rel_path})
                    )
                except Exception:
                    pass
    return docs
...
def save_review(rating, comment):
    try:
        ...
        if "file" in locals():
            repo.update_file(
                "reviews.csv", "Add new anonymous user review", csv_str, file.sha
            )
        else:
...
def show_feedback_form():
    ...
    with st.form("feedback_form", clear_on_submit=True):
        ...
        comment = st.text_area(
            "Your thoughts (optional)", placeholder="Works great! ..."
        )
        ...
        if submitted:
            with st.spinner("Saving securely..."):
                if save_review(rating, comment):
                    ...
                    st.error(
                        "Failed to save feedback right now. Please try again later."
                    )
...
def main():
    ...
    repo_url = st.sidebar.text_input(
        "Enter GitHub Repository URL", placeholder="https://github.com/user/repo"
    )
        ...
        with st.spinner(f"Cloning and analyzing {repo_url}..."):
            ...
            if vectorstore:
                ...
                st.session_state.messages = [
                    {
                        "role": "assistant",
                        "content": f"Repository `{repo_url}` loaded successfully! What would you like to know about the code?",
                    }
                ]
    ...
    if st.session_state.vectorstore:
        ...
        config_changed = (
            ...
            or st.session_state.get("ollama_model") != ollama_model
    ...
    with tab1:
        ...
        if st.session_state.vectorstore:
            if prompt := st.chat_input("Ask a question about the codebase..."):
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Thinking..."):
                        try:
                            lang_instruction = (
                                f"\n\n[System Requirement: You MUST provide your final response entirely in {language}.]"
                                if language != "English"
                                else ""
                            )
                            response = st.session_state.qa_chain(
                                {"question": prompt + lang_instruction}
                            )
            ...
            if not repo_url:
                st.info(
                    "👈 Please paste a GitHub Repository URL in the sidebar to load a codebase!"
                )
